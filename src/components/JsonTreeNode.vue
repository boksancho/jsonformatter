...
const valueClass = computed(() => {
  ...
  return map[type.value] ?? 'text-zinc-300'
})
</script>
...
<template>
  <div>
    <div class="flex items-center" :style="{ paddingLeft: `${depth * 16}px` }">
      <button
        v-if="isContainer"
        class="w-5 shrink-0 cursor-pointer text-center text-zinc-500 hover:text-zinc-300"
      ...
      <template v-if="keyName !== undefined">
        <span class="text-sky-300">"{{ keyName }}"</span>
      ...
      <template v-if="isContainer">
        ...
        <span
          class="ml-1.5 rounded bg-zinc-800 px-1.5 py-0.5 text-xs leading-4 text-zinc-400"
        >
