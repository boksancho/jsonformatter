  ...
  <div class="flex h-full flex-col">
    ...
    <div v-else-if="!result.ok" class="flex-1 overflow-auto p-4">
      <div class="rounded-md border border-rose-500/40 bg-rose-500/10 p-4">
        <p class="text-sm font-medium text-rose-600 dark:text-rose-400">Invalid JSON</p>
        <p class="mt-1 font-mono text-sm text-rose-500 dark:text-rose-300">
          {{ result.error.message }}
        </p>
        <p
          v-if="result.error.line !== undefined"
          class="mt-2 text-xs text-rose-500/80 dark:text-rose-400/80"
        >
